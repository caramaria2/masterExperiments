...
def stemming(tokens):
    text = [SnowballStemmer("english").stem(word) for word in tokens]
    return text
...
def naiveBayes(x_train, x_test, y_train, y_test, labelTitle):
    nb = MultinomialNB()
    writeLog("Naive Bayes, acc, recall, precision, f1")
    for x in labelTitle:
...
def j48(x_train, x_test, y_train, y_test, labelTitle):
    ...
    writeLog("J48, acc, recall, precision, f1")
    for x in labelTitle:
        result = trainClassifier(j48clf, x, x_train, x_test, y_train, y_test)
        results.append(result)

def logisticRegression(x_train, x_test, y_train, y_test, labelTitle):
    lr = LogisticRegression()
    writeLog("logisticRegression, acc, recall, precision, f1")
    for x in labelTitle:
        result = trainClassifier(lr, x, x_train, x_test, y_train, y_test)
        results.append(result)

def svm(x_train, x_test, y_train, y_test, labelTitle):
    bag = SVC()
    writeLog("SVM, acc, recall, precision, f1")
    for x in labelTitle:
        result = trainClassifier(bag, x, x_train, x_test, y_train, y_test)
        results.append(result)
...
path = "./logfiles/"
experimentID = "ID45_"
experiment = "SWR_ST_Sentiment+BOW"
timestamp = int(round(time.time() * 1000))
filename = path+experimentID+experiment +str(timestamp)+'.csv';
f = open(filename, "x")

writeLog("Preporcessing, SWR, Snowball Stemmer")
writeLog("Feature Extraction, TF (Count Vectorizer")
...
tokenized_data = tokenized_data.apply(lambda x: stemming(x)) #Stemming Snow

#detokenize data again
...
#join matrix
# joinedMatrix = sp.hstack((tf, sentiment_Array), format='csr')
...
logisticRegression(X_train, X_test, y_train, y_test, labelTitle)
