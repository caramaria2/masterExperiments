...
def bagging(x_train, x_test, y_train, y_test, labelTitle):
    bag = BaggingClassifier(base_estimator=SVC(),n_estimators=10, random_state=0)
    writeLog("Bagging, acc, recall, precision, f1")
    for x in labelTitle:
...
path = "./logfiles/"
experimentID = "ID19_"
experiment = "SWR_ST_LEM_TFIDF"
timestamp = int(round(time.time() * 1000))
filename = path+experimentID+experiment +str(timestamp)+'.csv';
f = open(filename, "x")

writeLog("Preporcessing, SWR, Lemmatization, Stemming")
writeLog("Feature Extraction, TFIDF)")
...
bagging(X_train, X_test, y_train, y_test, labelTitle)
...
writeLog("Bagging")
clf_chain_modelSVM =build_model(BaggingClassifier(), LabelPowerset, X_train, y_train, X_test, y_test)